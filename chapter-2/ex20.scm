...
(define (same-parity a . lat)
  (define (filter-list f lat filtered)
    (if (null? lat)
        (reverse filtered)
        (if (eq? #t (f (car lat)))
            (filter-list f (cdr lat) (cons (car lat) filtered))
            (filter-list f (cdr lat) filtered))))
  (let ((rem (remainder a 2)))
    (filter-list 
     (lambda (x) (eq? (remainder x 2) rem)) lat (list a))))
